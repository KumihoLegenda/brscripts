// ==UserScript==
// @name         BR Panel (Header Right Side)
// @namespace    http://tampermonkey.net/
// @version      3.2
// @description  Header buttons on the right side. Dynamic server selection, IP comparison tool.
...
(function () {
    ...
    if (document.body.getAttribute('data-br-script-header-right')) {
        return;
    }
    document.body.setAttribute('data-br-script-header-right', 'true');
    ...
    try {
        (function() {
            ...
            // --- Функция для создания кнопок в правой части шапки ---
            function createHeaderButtons() {
                // Ищем контейнер для кнопок в правой части шапки (как во втором скрипте)
                let targetContainer = document.querySelector('.p-nav-opposite');
                
                if (!targetContainer) {
                    console.warn('[BR Script] Не найден контейнер .p-nav-opposite');
                    return;
                }
                ...
                // Добавляем кнопки ЖБТ для выбранных серверов
                ...
                // Добавляем кнопки ТР для выбранных серверов
                selectedIds.forEach(id => {
                ...
                // Добавляем кнопки ЖБИ для выбранных серверов
                selectedIds.forEach(id => {
                ...
                // Добавляем контейнер в правую часть шапки
            ...
            style.textContent = `
                ...
                .br-header-buttons-container {
                    display: flex;
                    gap: 4px;
                    margin-left: auto;
                }
                
                /* На ПК - кнопки переносятся на следующую строку */
                ...
                /* На мобильных устройствах - горизонтальная прокрутка */
                @media (max-width: 768px) {
                    ...
                    .br-header-buttons-container::-webkit-scrollbar {
                        height: 3px;
                    }
                    
                    .br-header-buttons-container::-webkit-scrollbar-track {
                        background: #2a2a2a;
                        border-radius: 2px;
                    }
                    
                    .br-header-buttons-container::-webkit-scrollbar-thumb {
                        background: #555;
                        border-radius: 2px;
                    }
                }
            ...
            // --- Функции IP (сокращенные из второго скрипта) ---
            let ipModal = null;
            
            function openIPModal() {
                if (!ipModal) {
                    ipModal = createIPModal();
                    document.body.appendChild(ipModal);
                }
                loadIPSavedData();
                const resultSection = ipModal.querySelector('#ipResult');
            ...
            function createIPModal() {
                    ...
                    <div class="ip-modal-content">
                        ...
                        <div class="ip-inputs-row">
                            <div class="ip-input-container">
                                <label class="ip-input-label">Первый IP-адрес:</label>
                                <input type="text" class="ip-input" id="ipAddress1" placeholder="Введите первый IP-адрес">
                            </div>
                            <div class="ip-input-container">
                                <label class="ip-input-label">Второй IP-адрес:</label>
                ...
                modal.querySelector('#ipClearBtn').onclick = () => {
                    ...
                    const rs = document.getElementById('ipResult');
            ...
            function loadIPSavedData() {
                const ip1 = localStorage.getItem('forum_ip_data_1');
                const ip2 = localStorage.getItem('forum_ip_data_2');
                if (ip1) document.getElementById('ipAddress1').value = ip1;
                if (ip2) document.getElementById('ipAddress2').value = ip2;
            }
            ...
            async function compareIPAddresses() {
                ...
                if (!ip1 || !ip2) {
                    resultText.innerHTML = '<div class="ip-error">Введите оба IP-адреса</div>';
                    resultDiv.style.display = 'block';
                    return;
                }
                
                resultText.innerHTML = '<div class="ip-loading">Загрузка...</div>';
                resultDiv.style.display = 'block';
                
                try {
                    const [geo1, geo2] = await Promise.all([getGeoData(ip1), getGeoData(ip2)]);
                    if (!geo1 || !geo2) throw new Error('Не удалось получить данные');
                    
                    const distance = calculateDistance(geo1, geo2);
                    resultText.innerHTML = `
                        <div class="ip-comparison-result">
                            <div class="ip-comparison-header">Результат сравнения</div>
                            <div class="ip-comparison-grid">
                                <div class="ip-comparison-item">
                                    <div class="ip-comparison-title">IP 1: ${ip1}</div>
                                    <div>Страна: ${geo1.country}</div>
                                    <div>Город: ${geo1.city}</div>
                                    <div>Регион: ${geo1.region}</div>
                                    <div>Провайдер: ${geo1.isp}</div>
                                </div>
                                <div class="ip-comparison-item">
                                    <div class="ip-comparison-title">IP 2: ${ip2}</div>
                                    <div>Страна: ${geo2.country}</div>
                                    <div>Город: ${geo2.city}</div>
                                    <div>Регион: ${geo2.region}</div>
                                    <div>Провайдер: ${geo2.isp}</div>
                                </div>
                            </div>
                            <div class="ip-distance-result">
                                <div class="ip-distance-header">Расстояние</div>
                                <div class="ip-distance-value">${distance} км</div>
                            </div>
                            <div class="ip-comparison-summary">
                                <div>Совпадение страны: ${geo1.country === geo2.country ? '✅ Да' : '❌ Нет'}</div>
                                <div>Совпадение города: ${geo1.city === geo2.city ? '✅ Да' : '❌ Нет'}</div>
                            </div>
                        </div>
                    `;
                } catch(e) {
                    resultText.innerHTML = `<div class="ip-error">Ошибка: ${e.message}</div>`;
                }
            }
                        ...
                        if (resp.ok) {
                            ...
                            if (data.country || data.country_name) {
                                return {
                                    ...
                                    region: data.region || data.regionName || 'Неизвестно',
                                    isp: data.org || data.isp || 'Неизвестно',
            ...
            function calculateDistance(geo1, geo2) {
                if (!geo1.latitude || !geo2.latitude) return '?';
                const R = 6371;
                const dLat = (geo2.latitude - geo1.latitude) * Math.PI / 180;
                const dLon = (geo2.longitude - geo1.longitude) * Math.PI / 180;
                const a = Math.sin(dLat/2)**2 + Math.cos(geo1.latitude * Math.PI/180) * Math.cos(geo2.latitude * Math.PI/180) * Math.sin(dLon/2)**2;
            ...
            function init() {
                ...
                // Сохраняем введенные IP
                document.addEventListener('input', (e) => {
                    if (e.target.id === 'ipAddress1') localStorage.setItem('forum_ip_data_1', e.target.value);
                    if (e.target.id === 'ipAddress2') localStorage.setItem('forum_ip_data_2', e.target.value);
                });
            }
        ...
        console.error('[BR Script] Error:', e);
    }
})();
